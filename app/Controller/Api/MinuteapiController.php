...
class MinuteAPIController extends APIController {
  // ...
     ...
     if ($this->request->is('get')) 
      {
        ...
        if ($start) {   $start = date('Y-m-d H:i:s', strtotime($start));
                        $start_minute = date('i', strtotime($start));
         }
        if ($end)   {   $end = date('Y-m-d H:i:s', strtotime($end));
                        $end_minute = date('i', strtotime($end));
         }
        if( $type ){
            $time_list_q = $this->AccessLog->getTotalTapByContentsAndTime($this->session['team_id'], $_cid, "Q", $start, $end, $app, AccessLog::TYPE_MINUTE);
            $time_list_n = $this->AccessLog->getTotalTapByContentsAndTime($this->session['team_id'], $_cid, "N", $start, $end, $app, AccessLog::TYPE_MINUTE);
        }else{
            $this->AccessLog->loadActivatePalteList($this->session['team_id']);
            $time_list_q = $this->AccessLog->getTotalTapByTagAndTime($this->session['team_id'], $_tag, "Q", $start, $end, $app, AccessLog::TYPE_MINUTE);
            $time_list_n = $this->AccessLog->getTotalTapByTagAndTime($this->session['team_id'], $_tag, "N", $start, $end, $app, AccessLog::TYPE_MINUTE);
        }
    
        $time_list = array();
        
        for( $i=0; $i<60; $i++ ){
          $time_index = $i+$start_minute;
          if( $time_index > 59)
            $time_index -= 60;
        ...
        $this->result['minute_data'] = $time_list;
    }
  }
